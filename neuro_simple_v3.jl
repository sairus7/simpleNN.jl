# batch run converted into matrix operations,
# batch_size preallocation for nn layers
...
σ(z) = 1/(1+exp(-z)) # @inline ?
σ_grad(z) = σ(z)*(1-σ(z))

function (net::network_v3)(a)
...
function network_v3(sizes, batch_size)
    ...
    network_v3(num_layers, sizearr, biases, weights, zs, activations,∇_b,∇_w,δs)
end

function update_batch(net::network_v3, x, y, η)
    ∇_b = net.∇_b
    ∇_w = net.∇_w

    # not need to fill ∇_b ∇_w with zero!
...
function backprop!(net::network_v3, x, y)
    ∇_b = net.∇_b
    ∇_w = net.∇_w
...
function SGDtrain(net::network_v3, traindata, epochs, batch_size, η, testdata=nothing)
...
function evaluate(net::network_v3, x, y)
    hits = 0
    for i = 1:size(x, 2)
        out = net(x[:,i])
...
function main_v3()
    ...
    batch_size = 10
    η = 1.25
    net = network_v3([784, 30, 10], batch_size)
